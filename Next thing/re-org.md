...
Such unawareness is especially notable amongst users who operate primarily with small, predominantly private, collections of physical specimens in reachable drawers. In contrast, those who operate primarily either on recorded datasets of 10,000+ unique records, or collections intended for use by more than one person are more likely to be aware of the issues resulting from nomenclatural divergence.

To the user with access to the physical specimens, a nomenclatural divergence is a simple problem to solve - provided that they have the requisite knowledge. The informed user can reach into the drawer, pick out the specimen, and perform re-determination. They then add a label to the specimen and place the specimen back, safe in the knowledge that *they* know what the answer is. 

For the user of data, that entire process is hidden behind multiple layers of abstraction. There is currently no way to perform the 're-determine and label' process in a digital environment outside of individually correcting each existing usage of that piece of data - an absolutely impossible task. To complete such a task, one would need to scratch out and re-write every single use of a given name in every book, paper, journal, record card, Excel sheet, database etc etc.
